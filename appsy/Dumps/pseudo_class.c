...
static int __init plp_kmem_init(void)
{
	...
	//list_add_tail(&my_dev->list,&dev_list); //private list
        ...
        //allocated dynamically - we need to pass pointer to 
        //a variable of dev_t type 

        //second param. is the minor no. associated with the first
        //device no. allocated dynamically - ideally, we can just 
        //say 0 - meaning, allocate me the first device id, with 
        //minor number 0 - we can pass non-zero values, if required


        //third param. is the total no. of dynamic device ids. requested
        //
        //
        //last param. is a logical name - internally used, in the 
        //system 

        //if this system API succeeds, we will be allocated character
        //device ids in the range plp_kmem_dev -> plp_kmem_dev+ndevices-1,
        //with minor no. of the first device id (plp_kmem_dev)  
        // set to 0 !!!
        //so, we will get the following :
        //first device id ---->  plp_kmem_dev(value stored, in this variable)
        //second device id ---->  plp_kmem_dev+1
        //third device id ---->  plp_kmem_dev+2
        .
        .
        .
        //last device id ---->  plp_kmem_dev+ndevices-1 
        // 
        //
