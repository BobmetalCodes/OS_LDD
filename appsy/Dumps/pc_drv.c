...
/* Size of buffer used for FIFO */


static LIST_HEAD(dev_list);
/* Head node of the device list */
static unsigned int ndevices = 1;
/* Module parameter for number of devices */
static dev_t pc_devno;
/* Variable to store the first number in the allocated device range */
...
static int __init pc_drv_init(void)
{
	...
	struct proc_dir_entry *entry = NULL;
        //dump_stack() and tracing/backtrace enable us 
        //to understand and check the call backs 
	dump_stack();
	...
	if ((entry = create_proc_entry("pc_devinfo", S_IRUSR, NULL)) == NULL)
	{
