...
static ssize_t pcdd_read(struct file *file,const char __user *buff,ssize_t count,loff_t *pos)
{
	...
	printk("inside read call...\n");
        ...
        //
        //as we progress, we may find that kfifo and its buffer are
	...
	for(i=0;i<ndevices;i++)
	{
		...
		printk("5: %d spin_lock init\n", i);
	        //this spin lock requires an initialization !!!	
                ...
                //
		my_dev->kfifo = kfifo_init(my_dev->buff,MAX_BUFFSIZE,
                                           GFP_KERNEL,&(my_dev->my_lock));
